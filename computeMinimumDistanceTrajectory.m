function trajectory = computeMinimumDistanceTrajectory(p_i, p_f, rollerPositions, rollerRadii, wrapDirections, r_t, v_t)
    ...
    % Generate the arc from the last tangency point to the final point out
    ...
    % Interpolate trajectory for constant tangential speed
    trajectory = interpolateTrajectoryForMinimumTrajectory(trajectory, v_t);
end

function interpolatedTrajectory = interpolateTrajectoryForMinimumTrajectory(trajectory, v_t)
    ...
    for i = 1:size(trajectory, 1) - 1
        ...
        % Compute the number of points needed for the given tangential speed
        numPoints = ceil(distance / v_t);
